...
class Preprocessor():
    ...
    def __init__(self, max_vocab_size=10000, max_seq_len=300, **kwargs):
        ...
        self.context_question_seperator = kwargs.get('context_question_seperator', '<SEP>')
    ...
    def pad_sequence(self, sequence):
        return pad_sequences([sequence], maxlen=self.max_seq_len, padding='post')[0]

    def text_to_sequence(self, text, is_answer=False):
        if is_answer:
            return self.tokenizer.texts_to_sequences([self.answer_start_token + text + self.answer_end_token])[0]
        return self.tokenizer.texts_to_sequences([text])[0]

    def seq_to_text(self, seq):
        return self.tokenizer.sequences_to_texts([seq])[0]

    def fit_on_texts(self, contexts, questions, answers):
        combined_text = contexts + questions + answers
